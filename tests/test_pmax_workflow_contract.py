...
def test_final_contract_requires_all_63_operations_and_all_25_reads(tmp_path):
    ...
    assert 67 <= len(h.tool_names(server)) <= 76
    ro, _ = final_server(tmp_path, read_only=True)
    assert 27 <= len(h.tool_names(ro)) <= 30
    specs = all_tool_specs()
    assert 67 <= len({spec.name for spec in specs}) == len(specs) <= 76
...
def test_authored_requirements_and_declared_domains_are_complete(tmp_path):
    ...
    assert len(tools) == len(by_name) == 76 and set(by_name) == ALL_WRITE_MODE_TOOLS | PMAX_ADDITIONS | SEARCH_URL_ADDITIONS | TARGETING_ADDITIONS
...
def test_all_read_goldens_are_owned_and_final_parity_checks_the_merged_inventory(tmp_path):
    ...
    assert len(names) == len(set(names)) == 30 and set(names) == READ_TOOLS | PMAX_READS | SEARCH_URL_READS | TARGETING_READS
    server, _ = final_server(tmp_path)
...
def test_public_pmax_guide_is_linked_truthful_and_generated_reference_current(tmp_path):
    ...
    assert any(total in readme and reads in readme for total, reads in (("63", "25"), ("65", "26"), ("67", "27"), ("74", "29"), ("76", "30")))
    assert "docs/pmax.md" in (ROOT / "README.md").read_text()
    migration = (ROOT / "docs/migration.md").read_text()
    assert any(total in migration and reads in migration for total, reads in (("63", "25"), ("65", "26"), ("67", "27"), ("74", "29"), ("76", "30"))) and "pmax" in migration.lower()
