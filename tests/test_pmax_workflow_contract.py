...
from shared_targeting_oracle import ADDITIONS as TARGETING_ADDITIONS, READS as TARGETING_READS, FIXTURES as TARGETING_FIXTURES, NEW_SOURCE
...
def test_final_contract_requires_all_63_operations_and_all_25_reads(tmp_path):
    ...
    assert 76 <= len(h.tool_names(server)) <= 84
    ro, _ = final_server(tmp_path, read_only=True)
    assert 30 <= len(h.tool_names(ro)) <= 34
    specs = all_tool_specs()
    assert 76 <= len({spec.name for spec in specs}) == len(specs) <= 84
...
def test_authored_requirements_and_declared_domains_are_complete(tmp_path):
    ...
    assert len(tools) == len(by_name) == 84 and set(by_name) == ALL_WRITE_MODE_TOOLS | PMAX_ADDITIONS | SEARCH_URL_ADDITIONS | TARGETING_ADDITIONS | EXPERIMENT_ADDITIONS | {NETWORK_TOOL}
...
def test_all_read_goldens_are_owned_and_final_parity_checks_the_merged_inventory(tmp_path):
    ...
    directory.mkdir()
    fixtures = list(h.CONTRACT_DIR.glob("*.json")) + list(PMAX_FIXTURES.glob("*.json")) + list((ROOT / "tests/fixtures/search_urls").glob("*.json")) + list(TARGETING_FIXTURES.glob("*.json"))
    fixtures += list(EXPERIMENT_FIXTURES.glob("*.json"))
    names = [h.load_contract_fixture(path)["tool"] for path in fixtures]
    assert len(names) == len(set(names)) == 34 and set(names) == READ_TOOLS | PMAX_READS | SEARCH_URL_READS | TARGETING_READS | EXPERIMENT_READS
    server, _ = final_server(tmp_path)
    selected_inventory = {h.load_contract_fixture(path)["tool"] for path in load_script("parity").fixture_paths(None)}
    names = [name for name in names if name in h.tool_names(server) and (name in READ_TOOLS | PMAX_READS | SEARCH_URL_READS or name in selected_inventory)]
...
def test_source_archive_inventory_requires_new_oracles_goldens_and_guide(tmp_path):
    ...
    complete += [path for path in NEW_SOURCE if path in checker.REQUIRED_SOURCE]
...
def test_public_pmax_guide_is_linked_truthful_and_generated_reference_current(tmp_path):
    ...
    assert any(total in readme and reads in readme for total, reads in (("63", "25"), ("65", "26"), ("67", "27"), ("74", "29"), ("76", "30"), ("79", "33"), ("80", "33"), ("83", "34"), ("84", "34")))
    assert "docs/pmax.md" in (ROOT / "README.md").read_text()
    migration = (ROOT / "docs/migration.md").read_text()
    assert any(total in migration and reads in migration for total, reads in (("63", "25"), ("65", "26"), ("67", "27"), ("74", "29"), ("76", "30"), ("79", "33"), ("80", "33"), ("83", "34"), ("84", "34"))) and "pmax" in migration.lower()
